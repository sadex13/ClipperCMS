...
//			return FALSE;
	...
	function exportDir($dirid, $dirpath, &$i) {
		...
		//$rs = mysql_query($sql); // sadex13
		$rs = $modx->db->query($sql); // sadex13 $modx->db->getRow
		$dircontent = array();
		//while($row = mysql_fetch_assoc($rs)) { // sadex13
		while($row = modx->db->getRow($rs)) { // sadex13
...
//		print_r ($dircontent);
	...
	if($modx->config['friendly_urls']==1 && $modx->config['use_alias_path']==1) {
		...
		//$rs = mysql_query($sql); // sadex13
		$rs = $modx->db->query($sql); // sadex13
		//$row = mysql_fetch_row($rs); // sadex13
	...
	// Modified for export alias path  2006/3/24 end
		...
		//$rs = mysql_query($sql); // sadex13
		$rs = $modx->db->query($sql); // sadex13 
		//$limit = mysql_num_rows($rs); // sadex13 
		$limit = $modx->db->getRecordCount($rs); // sadex13  
		printf($_lang['export_site_numberdocs'], $limit);
